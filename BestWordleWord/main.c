...
int main() {
  ...
  input = (struct data *)malloc(
      (count) * sizeof(struct data)); // allocating memory for struct data

  count = readFile(filename, count,
                   input); // reading file and storing words in struct data

  sort(input, count);                       // sorting struct data
  setOccurences(count, input, &counter[0]); // getting occurences of each
  // letter
...
void findSecretWord(int count, char word[], int counter[], struct data *input) {
  ...
  int start = getOccurance(
      word[0], counter); // getting position of first letter in struct data
  ...
  for (int i = start; i < count; i++) {
    if (word[1] == input[i].word[1]) {
      ...
      int status = compareWords(
          word, input[i].word); // comparing words, sets capital letters in word
...
void findCommonLetters(char word[], char guess[]) {
  ...
  // for(int j=0;j<wordlength;j++)
  {
    for (int k = 0; k < wordlength; k++) {
      if (guess[k] == word[0] || guess[k] == word[1] || guess[k] == word[2] ||
          guess[k] == word[3] || guess[k] == word[4]) {
